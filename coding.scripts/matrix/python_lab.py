...
value_list = [ dict[key] for dict in dlist for key in dict if(key == k) ]
...
value_list_modified_1 = [ dict.get(k, "NOT PRESENT") for dict in dlist ] # <-- Use the same expression here
k = 'Frodo'
value_list_modified_2 = [ dict.get(k, "NOT PRESENT") for dict in dlist ] # <-- as you do here
