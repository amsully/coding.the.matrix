...
def create_voting_dict(strlist):
    ...
    """
    return { fullString.split()[0]:[int(dig) for dig in fullString.split() if dig in ['-1','0','1']] for fullString in strlist }
...
def policy_compare(sen_a, sen_b, voting_dict):
    ...
    """
    return sum( voting_dict[sen_a][index]*voting_dict[sen_b][index] for index in range(len(voting_dict[sen_a])) )
...
def most_similar(sen, voting_dict):
    ...
    for key in voting_dict:
        if key != sen and policy_compare(sen,key,voting_dict) > agreement:
            agreement = policy_compare(sen, key, voting_dict)
            politician = key
    return politician
...
dump = open('voting_record_dump109.txt')
vd = create_voting_dict(dump)
...
def find_average_similarity(sen, sen_set, voting_dict):
    ...
    Example:
        ...
        True
    """
    return sum( [policy_compare(sen,senC, voting_dict) for senC in sen_set] )/(len(sen_set))

newD = open('voting_record_dump109.txt')
party_set = { fullString.split()[0]:fullString.split()[1] for fullString in newD.readlines() }
senator_set = { senator for senator in vd }
best = float("inf")
the_man = ''
for senator in senator_set:
    if party_set[senator] == 'D' and abs(best) > find_average_similarity(senator, senator_set, vd):
        best = find_average_similarity(senator, senator_set, vd)
        the_man = senator
most_average_Democrat = the_man # give the last name (or code that computes the last name)
...
def find_average_record(sen_set, voting_dict):
    ...
    Example: 
        ...
        [0.0, 1.0, 1.0]
    """
    return [sum([voting_dict[sen][index] for sen in sen_set])/len(sen_set) for index in range(len( next(iter(voting_dict.values() )) ))] 

newDD = open('voting_record_dump109.txt')
party_set = { fullString.split()[0]:fullString.split()[1] for fullString in newDD.readlines() }
senator_set = { senator for senator in vd if party_set[senator] == 'D'}

average_Democrat_record = find_average_record(senator_set, vd) # give the vector as a list
...
def bitter_rivals(voting_dict):
    ...
    sen1 = ""
    sen2 = ""
    for mainSenator in voting_dict:
        dif = policy_compare( mainSenator, least_similar(mainSenator, voting_dict), voting_dict)
        if dif < mostExtremeVal:
            mostExtremeVal = dif
            sen1 = mainSenator
            sen2 = least_similar(mainSenator, voting_dict)
