...
def equal(u,v):
    ...
    for key in u.f:
        if u.f[key] != 0:
            if key not in v.f or u.f[key] != v.f[key]:
                return False
    for key in v.f:
        if v.f[key] != 0:
            if key not in u.f or v.f[key] != u.f[key]:
                return False
    return True
...
def add(u,v):
    ...
    for key in u.f:
        if key not in v.f: 
            answer.update({key:u.f[key]})
        else:
            total = u.f[key] + v.f[key]
            answer.update({key:total})
    for key in v.f:
        if key not in u.f: 
            answer.update({key:v.f[key]})
        else:
            total = u.f[key] + v.f[key]
            answer.update({key:total})
    return Vec(u.D,answer)
...
def dot(u,v):
    ...
    for key in u.f:
        if key in v.f:
            products.append( u.f[key] * v.f[key] )
    return sum(products)
...
def scalar_mul(v, alpha):
    ...
    if alpha == 0:
        ...
        for key in v.f:
            scalarVec.update( {key:v.f[key] * alpha} )
        return Vec(v.D, scalarVec)
